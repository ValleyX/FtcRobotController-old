...
@Autonomous(name = "RookieAutonomousLeftOther")
public class RookieAtonomousLeftOther extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (robotHardware.pipeline.markerPos == RobotHardware.SkystoneDeterminationPipeline.MarkerPos.Red)
        {
            ...
            sleep(500);
            Gyro.driveStraight(.3,3,0); // forward half an inch
            Gyro.turnToHeading(.3,41 /*screw you andrew*/,3000); // right 42 degrees, if it ever misses go to 41
            robotHardware.pinch.setPosition(robotHardware.openPinch);
            sleep(500);
            Gyro.turnToHeading(.3,0,3000); // turn back to going forward
            liftdriver.moveInches(5,.3);
            sleep(500);
            /*Gyro.driveStraight(.3,-4,0);*/
            sleep(500);

            Gyro.driveStraight(.3,61.5,0); // push cone out of way
            Gyro.driveStraight(.3,-8,0); // go back some
            sleep(500);
            Gyro.turnToHeading(.3,-90,3000); // turn to cone stack
            liftdriver.moveInches(6,.5); //raise to cone stack
            Gyro.driveStraight(0.3,20,-90);// drive to cone stack
            robotHardware.pinch.setPosition(robotHardware.closedPinch);
            sleep(1000); // pause 1 sec
            liftdriver.moveInches(22,0.5);
            Gyro.driveStraight(0.3,-20,-90); // back up from cone grab
            Gyro.turnToHeading(0.3,138, 10000000); // rotate from cone
            robotHardware.pinch.setPosition(robotHardware.openPinch);
            sleep(100);
