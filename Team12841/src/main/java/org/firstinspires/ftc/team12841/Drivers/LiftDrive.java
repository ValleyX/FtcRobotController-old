...
public class LiftDrive
{
                            ...
                            boolean waiting) //are we returned only when complete?
    {
        ...
        if (robot_.OpMode_.opModeIsActive()) {
            ...
            robot_.OpMode_.telemetry.addData("Path0", "lift position at %7d ",
                    robot_.LiftMotor.getCurrentPosition());
            robot_.OpMode_.telemetry.update();

            // Determine new target position, and pass to motor controller
            newLiftTarget = robot_.LiftMotor.getCurrentPosition() + (int) (Inches * robot_.LiftCOUNTS_PER_INCH);

            robot_.LiftMotor.setTargetPosition(newLiftTarget);
            ...
            robot_.LiftMotor.setPower(Math.abs(speed));
            ...
            if (waiting_)
            {
                      ...
                      !IsActionDone())
                {
                    ...
                    robot_.OpMode_.telemetry.addData("Path1", "Running to %7d ",
                            newLiftTarget);
                    robot_.OpMode_.telemetry.addData("Path2", "Running at %7d ",
                            robot_.LiftMotor.getCurrentPosition());
                    robot_.OpMode_.telemetry.update();
                    robot_.OpMode_.idle();
                }
    ...
    public boolean IsActionDone()
    {
      //  return !robot_.LfMotor.isBusy() && !robot_.LbMotor.isBusy() && !robot_.RfMotor.isBusy() && !robot_.RbMotor.isBusy();
        return !robot_.LiftMotor.isBusy();
    }
//Hello people
    //stop the motors
