...
@Autonomous(name="RedWarehouseSideComplex ")
public class RedWarehouseSideComplex extends LinearOpMode {
    @Override
       ...
       // headingdrive.gyroDrive(.5,4,heading);
        ...
        if (elapsedTimestart.seconds() <= 20) {
            ...
            while (opModeIsActive() /*&& (width >= 120 || width < 20)*/ && elapsedTime.seconds() <= 3) {
               ...
               // while (elapsedTime.seconds() < 1 && !robot.redPipeline.isFound()) {
                while (heading > -60 && !robot.redPipeline.isFound()) {
                    robot.leftBack.setPower(-motorspeed);
                    robot.leftFront.setPower(-motorspeed);
                    robot.rightBack.setPower(motorspeed);
                    robot.rightFront.setPower(motorspeed);
                ...
                elapsedTime.reset();
               // while (elapsedTime.seconds() < 2 && !robot.redPipeline.isFound()) {
                while (heading < -30 && !robot.redPipeline.isFound()) {
                    robot.leftBack.setPower(motorspeed);
                    robot.leftFront.setPower(motorspeed);
                    robot.rightBack.setPower(-motorspeed);
                    robot.rightFront.setPower(-motorspeed);
                ...
                if (robot.redPipeline.isFound()) {
                    ...
                    if (robot.redPipeline.getXPosition() > 350 && heading <= -20) {
                        robot.leftBack.setPower(motorspeed);
                        robot.leftFront.setPower(motorspeed);
                        robot.rightBack.setPower(-motorspeed);
                        robot.rightFront.setPower(-motorspeed);
                    } else if (robot.redPipeline.getXPosition() < 325 && heading >= -50) {
                        robot.leftBack.setPower(-motorspeed);
                        robot.leftFront.setPower(-motorspeed);
                        robot.rightBack.setPower(motorspeed);
                        robot.rightFront.setPower(motorspeed);
                        ...
                        robot.rightBack.setPower(motorspeed);
                        robot.rightFront.setPower(motorspeed);
                        System.out.println("ValleyX: Aligned ");
                        aligned = true;
                        break;
                    }
            ...
            if (!aligned)
            {
                headingdrive.gyroTurn(0.4,-35);
            }
            double maxcreepinches = 12;
            double beforecreep = robot.leftFront.getCurrentPosition();
            double maxcreep = beforecreep + (maxcreepinches* robot.COUNTS_PER_INCH);
            robot.leftFront.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            sleep(500);
            System.out.println("ValleyX: width " + robot.redPipeline.getFoundRect().width + " current " + robot.leftFront.getCurrentPosition() + " target " + maxcreep);
            double loopwidth;
            while (((loopwidth = robot.redPipeline.getFoundRect().width) <= 30) && (robot.leftFront.getCurrentPosition() < maxcreep) )
            {
                    robot.allpower(0.05);
                    telemetry.addData("where am i ", robot.leftFront.getCurrentPosition() );
                    telemetry.addData("where do i want to go ", maxcreep);
                    telemetry.addData("current width ",loopwidth);
                    telemetry.update();
                    System.out.println("ValleyX: width " + loopwidth + " current " + robot.leftFront.getCurrentPosition() + " target " + maxcreep);
            }


            robot.allpower(0);
            if (robot.leftFront.getCurrentPosition() >= maxcreep)
            {
                System.out.println("ValleyX: stopped due to distance");
            }

            heading = -robot.imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES).firstAngle;
            //headingdrive.gyroTurn(0.4,heading- 10);
 ...
 */
         ...
         */
            ...
            headingdrive.gyroTurn(1, -85);

            headingdrive.gyroDrive(1, -55, -85);

            liftto.LiftToDistance(0.8 , -5, true);

            headingdrive.gyroTurn(1,-180);
